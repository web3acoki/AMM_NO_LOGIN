  ...
  <form @submit.prevent>
    ...
    <div class="mb-3">
      <div class="alert alert-info">
        比如最高价格20$,最低价格10$,使用市值机器人购买代币将价格拉升至20$后,开始卖出将价格降低至10$,又开始买入将代币价格拉升至20$,一直这样重复执行
      </div>
    </div>

    <!-- 震荡价格区间 -->
    <div class="mb-3">
      <h6 class="fw-semibold mb-2">震荡价格区间</h6>
      <div class="input-group">
        <input type="number" class="form-control" v-model.number="lowPrice" />
        <span class="input-group-text">~</span>
        <input type="number" class="form-control" v-model.number="highPrice" />
      </div>
      <div class="small text-muted mt-1">
        例如填写10-20,代币价格会在10$-20$区间重复震荡
      </div>
    </div>
...
// 震荡价格区间
const lowPrice = ref<number>(10);
const highPrice = ref<number>(20);
